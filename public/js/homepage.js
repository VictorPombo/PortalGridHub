...
async function loadLiveNews() {
  ...
  try {
    ...
    const pipelineFeed = [...(data.br || []), ...(data.global || [])];
...
function renderHeroGrid() {
  ...
  const lM = aM.isReal ? aM.link : '/materia.html?id=' + aM.slug;
  const lS1 = aS1.isReal ? aS1.link : '/materia.html?id=' + aS1.slug;
  const lS2 = aS2.isReal ? aS2.link : '/materia.html?id=' + aS2.slug;
...
function renderNewsGrid() {
  ...
  grid.innerHTML = displayArticles.map((a, i) => {
    ...
    const isFeat = i === 0;
    const lUrl = a.isReal ? a.link : '/materia.html?id=' + a.slug;
