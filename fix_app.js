...
// 1. Remove HERO_DATA
code = code.replace(/\/\* ══ HERO_DATA.*?\}\s*;/s, '');

// 2. Remove updateHero
code = code.replace(/\/\* ══ updateHero.*?\}\n/s, '');

// 3. Update filterCat
code = code.replace(/function filterCat\(cat\)\{.*?\n\}/s, `function filterCat(cat){
...
const newRenderHero = `function renderHeroGrid(catFilter = 'all') {
  ...
  if (catFilter === 'all') {
    ...
    // Determine exact category articles
    ...
    // If not enough news for hero, just hide the hero entirely and show normal cards
    if (catArts.length < 3) {
...
code = code.replace(/function renderHeroGrid\(\) \{.*?\n\}/s, newRenderHero);
